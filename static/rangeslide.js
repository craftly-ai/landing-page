;(function (root, factory) {
  ...
  Thumb.prototype = {
    ...
    getRight: function () {
      return (
        this.__element && this.__element.offsetLeft + this.__element.offsetWidth
      )
    },
  ...
  Rangeslide.prototype = {
    init: function (target, config) {
      ...
      this.__playInterval
      this.__thumbLeft = new Thumb(
        this.getMarkerByIndex(this.config.startPosition),
        this.config.startPosition
      )
      this.__thumbRight = new Thumb(
        this.getMarkerByIndex(this.config.endPosition),
    ...
    getSelection: function () {
      ...
      var selectedElements = this.__targetElement.querySelectorAll(
          ".track-marker.selected"
        ),
        selectSet = []

      for (var i = 0; i < selectedElements.length; i++) {
        selectSet.push(
          this.config.data[parseInt(selectedElements[i].dataset.index)]
        )
      }
    ...
    setValueByIndex: function (index, thumb) {
      ...
      var distance =
        (this.__targetElement.clientWidth - distanceOffset) /
        ((this.config.data.length - 1) / this.config.stepSize)
    ...
    setMaxValueByIndex: function (index) {
      if (this.isSingleMode())
        return this.setValueByIndex(index, this.__thumbLeft)
      if (this.isSelectMode()) return
    ...
    setMaxValueByAttribute: function (attributeName, attributeValue) {
      ...
      if (this.isSingleMode())
        return this.setValueByIndex(l - 1, this.__thumbLeft)
    ...
    __loadStoredValues: function () {
      if (
        Object.keys(this.__valuesStore).length === 0 &&
        this.__valuesStore.constructor === Object
      )
        return
    ...
    __setActiveLabel: function (index) {
      var previouslyActiveLabel = this.__targetElement.querySelector(
        ".tick-label.selected"
      )
      var currentlyActiveLabel = this.__targetElement.querySelector(
        "#" + this.__targetElement.id + "_label" + index
      )
    ...
    __getThumbStart: function (thumbElement) {
      return isNaN(parseFloat(thumbElement.style.left))
        ? 0
    ...
    __setAutoPlay: function (value) {
      ...
      if (!value) {
        clearInterval(this.__playInterval)
        this.fire("playStop", [
          this.__thumbLeft.getValue(),
          this.__targetElement,
        ])
        return
      }
      this.__playInterval = setInterval(
        this.__goToNextMarker.bind(this),
    ...
    __onTrackClicked: function (event) {
      this.config.slideMode === "free"
        ? this.__place(event.offsetX)
    ...
    __onMarkerClicked: function (event) {
      if (this.isSelectMode()) {
        ...
        var positionX =
          event.currentTarget.offsetLeft + this.config.markerSize / 2
        var index = parseInt(event.currentTarget.dataset.index)
        var thumb = this.__getClosestThumbElement(positionX)
        this.config.slideMode === "free"
          ? this.setValueByIndex(index, thumb)
          : this.__snap(positionX)
      }
    ...
    __onThumbMouseUp: function (event) {
      ...
      this.config.slideMode === "snap"
        ? this.__snap(thumb.start)
        : this.__place(thumb.start, thumb)
      ...
      if (this.config.showTrackProgress) {
        if (this.isRangeMode()) {
          if (thumb.getElement().classList.contains(MAX_THUMB_CLASS)) {
            ...
            this.__trackProgressElement.style.width =
              newLeft - this.__thumbLeft.getLeft() + "px"
          } else if (thumb.getElement().classList.contains(MIN_THUMB_CLASS)) {
            this.__trackProgressElement.style.left = newLeft + "px"
            this.__trackProgressElement.style.width =
              this.__thumbRight.getLeft() - newLeft + "px"
          }
        } else {
    ...
    __onUpdateMarkersProgress: function () {
      ...
      this.isRangeMode()
        ? this.__updateMarkerRange()
        : this.__updateMarkerProgress()
    },
    ...
    __onMouseWheel: function (event) {
      ...
      if (this.isSingleMode()) {
        this.setValueByIndex(
          this.__thumbLeft.getIndex() + delta,
          this.__thumbLeft
        )
      } else if (this.isRangeMode()) {
    ...
    __place: function (positionX, thumb, noAnimation) {
      var closestThumbElement = this.isRangeMode()
        ? thumb || this.__getClosestThumbElement(positionX)
        : this.__thumbLeft
      var newValue = {}
      newValue[this.config.dataSource] = this.__getValueAtPosition(positionX)
      this.config.animations &&
        !noAnimation &&
        closestThumbElement.getElement().classList.add("animated")
      ...
      if (this.config.showTrackMarkersProgress) {
        if (!this.config.animations || noAnimation) {
          ...
          this.__onUpdateMarkersProgress()
          clearInterval(this.__markersUpdateInterval)
          this.__markersUpdateInterval = setInterval(
            this.__onUpdateMarkersProgress,
            20
          )
        }
      ...
      if (this.config.showTrackProgress) {
        this.config.animations &&
          !noAnimation &&
          this.__trackProgressElement.classList.add("animated")
        if (this.isRangeMode()) {
          if (
            closestThumbElement.getElement().classList.contains(MAX_THUMB_CLASS)
          ) {
            this.__trackProgressElement.style.left = this.__thumbLeft.getLeft()
            this.__trackProgressElement.style.width =
              positionX - this.__thumbLeft.getLeft() + "px"
          } else if (
            closestThumbElement.getElement().classList.contains(MIN_THUMB_CLASS)
          ) {
            this.__trackProgressElement.style.left = positionX + "px"
            this.__trackProgressElement.style.width =
              this.__thumbRight.getLeft() - positionX + "px"
          }
        } else {
      ...
      if (this.config.animations && !noAnimation) {
        closestThumbElement
          .getElement()
          .addEventListener("webkitTransitionEnd", this.__onThumbTransitionEnd)
        closestThumbElement
          .getElement()
          .addEventListener("otransitionend", this.__onThumbTransitionEnd)
        closestThumbElement
          .getElement()
          .addEventListener("oTransitionEnd", this.__onThumbTransitionEnd)
        closestThumbElement
          .getElement()
          .addEventListener("msTransitionEnd", this.__onThumbTransitionEnd)
        closestThumbElement
          .getElement()
          .addEventListener("transitionend", this.__onThumbTransitionEnd)
      }
      ...
      if (this.config.spacing === "data-driven") {
        ...
        if (progressToNextMarker < 0.5) {
          ...
          if (nextMarker <= this.config.data.length - 1) {
            newLeft = (
              positionX +
              distance * (1 - progressToNextMarker)
            ).toFixed()
            closestThumbElement.setLeft(newLeft)
          } else {
            newLeft = (positionX - distance * progressToNextMarker).toFixed()
    ...
    __createUI: function () {
      ...
      this.isRangeMode() &&
        this.__thumbRight.setElement(this.__createThumbElement(MAX_THUMB_CLASS))

      this.__sliderElement = this.__createSliderElement(
        this.__trackElement,
        this.__trackProgressElement,
        [this.__thumbLeft.getElement(), this.__thumbRight.getElement()]
      )
      ...
      if (this.config.showLabels) {
        if (this.config.labelsPosition === "above") {
          ...
          this.__createLabels(
            this.__labelContainerElementTop,
            true,
            !this.config.startAlternateLabelsFromTop,
            true
          )
      ...
      if (this.config.showValue) {
        ...
        this.isRangeMode() &&
          this.__thumbRight.addLabel(this.__createValueElement())
      }
      ...
      if (this.config.leftLabel) {
        ...
        this.__targetElement.style.marginLeft =
          this.config.sideLabelsWidth + "px"
      }

      if (this.config.rightLabel) {
        this.__rightLabelElement = this.__createSideLabel(
          this.config.rightLabel
        )
        fragment.appendChild(this.__rightLabelElement)
        this.__rightLabelElement.style.right =
          -this.config.sideLabelsWidth + "px"
        this.__targetElement.style.marginRight =
          this.config.sideLabelsWidth + "px"
      }
    ...
    __createTrackElement: function () {
      ...
      trackElement.className =
        "track noselect" + (this.config.animations ? " animated" : "")
    ...
    __createThumbElement: function (className) {
      ...
      thumbElement.style.marginTop =
        -this.config.thumbHeight / 2 - this.config.trackHeight / 2 + "px"
    ...
    __createSliderElement: function (
      trackElement,
      trackProgressElement,
      thumbElements
    ) {
    ...
    __createValueElement: function () {
      ...
      if (this.config.valueIndicatorPosition === "thumb") {
        ...
        valueElement.style.left =
          -this.config.valueIndicatorWidth / 2 +
          this.config.thumbWidth / 2 +
          "px"
      }
      ...
      if (this.config.valueIndicatorPosition === "above") {
        valueElement.style.top =
          -this.config.valueIndicatorHeight / 2 -
          this.config.thumbHeight / 2 -
          this.config.trackHeight -
          this.config.valueIndicatorOffset +
          "px"
    ...
    __createLabels: function (
      ...
      var labelOffset =
        this.config.labelsWidth / (everyOtherLabel ? 1 : 2) -
        this.config.thumbWidth / 2
      var distanceOffset = this.config.thumbWidth
      ...
      for (
        var i = skipFirst ? 1 : 0;
        i <= total;
        everyOtherLabel ? (i += 2) : i++
      ) {
        var labelElement = document.createElement("div")
        labelElement.className = "tick-label noselect"
        labelElement.style.width =
          this.config.labelsWidth * (everyOtherLabel ? 2 : 1) + "px"
        ...
        if (this.config.spacing === "data-driven") {
          var currentDistance =
            ...
            distance +
            " * " +
            this.__getDistance(this.__getDataContent(this.config.data[i])) +
            " - " +
            labelOffset +
            "px)"
          ...
          labelElement.style.left =
            "calc(" + distance + " * " + i + " - " + labelOffset + "px)"
        }
      ...
      if (labelsBeforeTicks) {
        parent.style.height =
          14 + (this.config.showTicks ? this.config.tickHeight : 0) + "px"
      }
    },
    ...
    __createTrackMarkers: function () {
      ...
      while (l--) {
        ...
        markerElement.style.top =
          -this.config.markerSize / 2 + this.config.trackHeight / 2 + "px"
        if (this.config.enableMarkerClick) {
          markerElement.onclick = this.__onMarkerClicked
          markerElement.style.cursor = "pointer"
        }
        if (this.config.spacing === "data-driven") {
          var currentDifference = this.__getDistance(
            this.__getDataContent(this.config.data[l])
          )
          ...
          markerElement.style.left =
            "calc(" + distance + " * " + l + " - " + markerOffset + "px)"
        }
    ...
    __attachMouseWheelHandlers: function () {
      if (this.__targetElement.addEventListener) {
        this.__targetElement.addEventListener(
          "mousewheel",
          this.__onMouseWheel,
          false
        )
    ...
    __detachMouseWheelHandlers: function () {
      if (this.__targetElement.removeEventListener) {
        this.__targetElement.removeEventListener(
          "mousewheel",
          this.__onMouseWheel,
          false
        )
    ...
    __attachTooltips: function (markers) {
      ...
      while (l--) {
        ...
        tooltipElement.innerText = this.__getTooltipsContent(
          this.config.data[l]
        )
    ...
    __getValueAtPosition: function (positionX) {
      ...
      var minValue = Math.min.apply(
        ...
        this.config.data.map(
          function (item) {
            return this.__getDataContent(item)
          }.bind(this)
        )
      )
      ...
      var minValueInPixels =
        (this.__trackMarkerElements &&
          this.__trackMarkerElements[0].offsetLeft) ||
        0
      var maxValueInPixels =
        (this.__trackMarkerElements &&
          this.__trackMarkerElements[this.config.data.length - 1].offsetLeft) ||
        0
    ...
    __getClosestMarker: function (positionX) {
      ...
      while (l--) {
        if (
          !closestMarker ||
          Math.abs(this.__trackMarkerElements[l].offsetLeft - positionX) <
            Math.abs(closestMarker.offsetLeft - positionX)
        ) {
          closestMarker =
            this.__trackMarkerElements && this.__trackMarkerElements[l]
        }
      }
    ...
    __adjustConfiguration: function (config) {
      var total = this.config.data.length - 1
      this.config.endPosition =
        this.config.endPosition > total ? total : this.config.endPosition
      if (this.isRangeMode()) {
    ...
    __getSmallestDistance: function (offset) {
      if (this.config.spacing === "data-driven") {
        ...
        return (
          "((100% - " + offset + "px) / " + (this.config.data.length - 1) + ")"
        )
      }
    },
    ...
    __updateMarkerProgress: function () {
      ...
      while (l--) {
        if (
          this.__trackMarkerElements[l].offsetLeft <= this.__thumbLeft.getLeft()
        ) {
    ...
    __updateMarkerRange: function () {
      ...
      while (l--) {
        if (
          this.__trackMarkerElements[l].offsetLeft >=
            this.__thumbLeft.getLeft() &&
          this.__trackMarkerElements[l].offsetLeft <=
            this.__thumbRight.getRight()
        ) {
          this.__trackMarkerElements[l].classList.add("completed")
        } else {
          this.__trackMarkerElements[l].classList.remove("completed")
    ...
    isPositionOutOfBounds: function (position) {
      ...
      var distanceBetweenLabels =
        (this.__targetElement.clientWidth - this.config.thumbWidth) / size
      return position > distanceBetweenLabels * size || position < 0
    },

    areThumbsTooClose: function (position) {
      var distanceBetweenThumbs =
        this.__thumbRight.getLeft() - this.__thumbLeft.getRight()
